...
const createWaypointView = ({ waypoint, destinations, offers }) => {
  ...
  const pointDestination =
    destinations.find(({ id }) => id === destination) || {};
  const pointTypeOffers = offers.find((offer) => offer.type === type).offers;
  const selectedOffers = pointTypeOffers.filter(({ id }) =>
    offerIds.includes(id)
  );

  return `<li class="trip-events__item">
      <div class="event">
        <time class="event__date" datetime="${getDateStringFromDate(dateFrom)}">
        ${humanizeDay(dateFrom)}</time>
        ...
        <div class="event__schedule">
          <p class="event__time">
            ...
            ${getTimeStringFromDate(dateFrom)}</time>
            &mdash;
            <time class="event__end-time" datetime="${dateTo}">
            ${getTimeStringFromDate(dateTo)}</time>
          </p>
